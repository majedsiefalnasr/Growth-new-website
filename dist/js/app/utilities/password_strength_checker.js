...
export var password_strength_checker = function (target) {
  // Check if run for target
  target
    ? // Add to parallax
      run(target)
    : // Loop all elements
      SUtility.each(document.querySelectorAll('[data-pass-strength]'), (elem) => run(elem));
};

// Run
function run(elem) {
  let elem_input = document.querySelector(
    'input[id="' + SUtility.attr(elem, 'data-pass-strength') + '"]'
  );

  // Check if password strength field has target
  if (!elem_input) return;

  let password_progress = elem.querySelector('.progress-bar');

  // let password_note = elem.querySelector('.notes'),
  ...
  elem_input.addEventListener('keyup', function () {
    ...
    let checked = zxcvbn(elem_input.value),
